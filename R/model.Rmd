...
```{r check}
pbp_test2$xG <- predict(xg_fit, pbp_test2, type = "prob")$.pred_1

avg_xG_by_coord <- pbp_test2 %>% group_by(x_coord, y_coord) %>%
...
```{r roc}
roc_tbl<-roc_curve(data = pbp_test2, is_goal, xG, event_level = 'second')
...
This gives us an auc of `r roc_auc(pbp_test2, is_goal, xG, event_level = 'second')`.

# Look at the Data
Lets take a look at the data from the test year: First player lists
```{r players}
pbp_test2$is_goal<-as.integer(pbp_test2$is_goal)-1  # convert is_goal back to 1/0 integer instead of factor. -1 because factors are 1 indexed...
xg_player <- pbp_test2 %>%
